...
export default function MolarityCalculator() {
  ...
  const calculateMolarity = () => {
    ...
    if (n < 0 || v <= 0) {
      setError('Moles must be non-negative and volume must be positive');
      return;
    }
  ...
  return (
    <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-2xl p-6 sm:p-8 border-2 border-primary-200 dark:border-primary-700">
      <div className="space-y-6">
        ...
        {/* Volume Input */}
        <div>
          <label htmlFor="volume" className="block text-sm font-semibold text-gray-700 dark:text-gray-300 mb-2">
            Volume (V)
          </label>
          <input
            type="number"
            ...
            step="any"
            className="w-full px-4 py-3 border-2 border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-primary-500 dark:bg-gray-700 dark:text-white text-lg transition-colors"
            aria-label="Volume of solution in liters"
          />
          <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">Unit: L (liters)</p>
        </div>
        ...
        {molarity !== null && (
          <div className="bg-gradient-to-br from-primary-50 to-accent-50 dark:from-primary-900/30 dark:to-accent-900/30 rounded-xl p-6 border-2 border-primary-300 dark:border-primary-600 animate-zoom-in">
            <div className="text-center">
              <p className="text-sm font-semibold text-gray-600 dark:text-gray-400 mb-2">Molarity</p>
              <p className="text-4xl font-bold text-primary-700 dark:text-primary-400 mb-1">
                {molarity.toFixed(4)}
              </p>
              <p className="text-lg text-gray-700 dark:text-gray-300 font-medium">mol/L (M)</p>
              <div className="mt-4 pt-4 border-t border-primary-200 dark:border-primary-700">
                <p className="text-sm text-gray-600 dark:text-gray-400">
                  This solution has a molarity of <strong>{molarity.toFixed(4)} M</strong>
                </p>
              </div>
            </div>
          </div>
